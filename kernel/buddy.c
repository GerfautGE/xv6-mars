...
void bd_print() {
    for (int k = 0; k < nsizes; k++) {
        printf("size %d (blksz %d nblk %d): free list: ", k, (int)BLK_SIZE(k),
               NBLK(k));
...
void bd_init(void *base, void *end) {
    ...
    // printf("bd: memory sz is %d bytes; allocate an size array of length
    ...
    if (free != BLK_SIZE(MAXSIZE) - meta - unavailable) {
        printf("free %d %d\n", free,
               (int)(BLK_SIZE(MAXSIZE) - meta - unavailable));
